...
<section class={`${panelClass} flex flex-col gap-3`}>
	<div class="flex items-start justify-between gap-3">
		...
		<div
			class="inline-flex min-h-7 min-w-7 items-center justify-center rounded-full border border-zinc-600 bg-zinc-800 px-2.5 text-xs font-bold text-zinc-300">
			{servers.length}
		</div>
	</div>
	...
	{#if loading}
		<div
			class="flex min-h-24 items-center justify-center rounded-lg border border-dashed border-zinc-600 bg-zinc-800/70 p-3 text-center text-sm text-zinc-400">
			Loading servers...
		</div>
	{:else if error}
		<div
			class="flex min-h-24 items-center justify-center rounded-lg border border-dashed border-red-500/70 bg-red-950/30 p-3 text-center text-sm text-red-300">
			{error}
		</div>
		...
		<div class="grid gap-2.5">
			{#each servers as server}
				<a class="block text-inherit no-underline" href={appHref(`/servers/${server.ID}`)}>
					<article
						class={`${chipClass} flex flex-col gap-3 p-3 transition-colors hover:border-zinc-500 hover:bg-zinc-800`}>
							...
							{#if (server.Tags ?? []).length > 0}
								<div class="flex flex-wrap justify-start gap-1.5 md:justify-end">
									{#each server.Tags ?? [] as tag}
										<div
											class="inline-flex items-center rounded-full border border-zinc-600 bg-zinc-800 px-2 py-0.5 text-[11px] font-bold text-zinc-300">
											{tag.Name}
										</div>
									{/each}
								</div>
						...
						<div class="flex flex-wrap gap-2">
							<div
								class="inline-flex items-center rounded-full bg-zinc-800/80 px-2 py-0.5 text-[11px] font-bold text-zinc-300 gap-1 ring-1 ring-inset ring-zinc-700/70">
